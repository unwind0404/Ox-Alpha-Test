...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const validModes: Array<'templates' | 'drafts' | 'llm'> = ['templates', 'drafts', 'llm']
  // Если mode не передан или невалиден — fallback на 'templates' (default в БД)
  // Если передан явно невалидный — 400
  if (mode !== undefined && mode !== null && mode !== '' && !validModes.includes(mode as 'templates' | 'drafts' | 'llm')) {
    return res.status(400).json({ error: 'mode: templates, drafts или llm' })
  }
  const validMode: 'templates' | 'drafts' | 'llm' = (mode && validModes.includes(mode as 'templates' | 'drafts' | 'llm'))
    ? (mode as 'templates' | 'drafts' | 'llm')
    : 'templates'
